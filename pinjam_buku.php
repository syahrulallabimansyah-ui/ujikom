...
$anggota_list = [];
$anggota_res  = mysqli_query($conn,
    "SELECT id, full_name, no_anggota, kelas, username
     FROM users
...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar-btn.active { background:rgba(255, 255, 255, 0.18); }
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="total-badge">📚 <?= $total ?> Tersedia</div>
  <div class="total-badge" style="margin-top:-4px;">📖 <?= $total_dipinjam ?> Dipinjam</div>
  ...
  <a class="sidebar-btn active" href="daftar_anggota.php">
    ...
    <?php
// ════════════════════════════════════════════════════════════
//  PATCH: Tambahkan 2 tombol ini ke dalam <aside class="sidebar">
//  di halaman_admin.php, setelah tombol "Edit Profil" yang ada.
//
//  Cari baris ini di halaman_admin.php:
...
<main class="main">
  ...
  <form method="GET" action="">
    <div class="topbar">
      ...
      <input type="text" name="q" placeholder="Cari buku untuk dipinjam…"
             value="<?= htmlspecialchars($search) ?>"/>
      <button type="submit" class="btn-search">Cari</button>
      <?php if ($search): ?>
      <a href="pinjam_buku.php" style="font-size:.75rem;color:var(--muted);text-decoration:none;white-space:nowrap;">✕ Reset</a>
      <?php endif; ?>
    </div>
  </form>
...
function renderAnggotaList(list) {
  ...
  wrap.innerHTML = list.map(a => {
    ...
    return `
      <button type="button" class="anggota-item" onclick="pilihAnggota(${a.id}, '${escapeAttr(a.full_name)}')">
        <div class="anggota-avatar">${inisial}</div>
